...
parser = argparse.ArgumentParser(description='Make time consumption plots.')
...
if __name__ == "__main__":
    if D == 2:
        if WITH_WL:
            df = pd.read_csv("logs_watershed/csv/proc_time_WL.csv", header=[0,1], index_col=0)
        else:
            df = pd.read_csv("logs_watershed/csv/proc_time.csv", header=[0,1], index_col=0)
    elif D == 3:
        if WITH_WL:
            df = pd.read_csv("logs_watershed/3D/csv/3D_proc_time_WL.csv", header=[0,1], index_col=0)
        else:
            df = pd.read_csv("logs_watershed/3D/csv/3D_proc_time.csv", header=[0,1], index_col=0)
    if WITH_WL:
    ...
    for cluster in clusters:
        ...
        for lib in level0:
            ...
            table.plot(x='size', y='time', style='--', marker='.', grid=True, ax=ax, logy=LOGARIFMIC, logx=LOGARIFMIC)
        ...
        if WITH_WL:
            ...
            # avizo_fast_estimations = [, , , ]
            # ax.plot([64**3, 151**3, 214**3, 253**3], avizo_fast_estimations, linestyle='--', marker='.', color='k')
            # titles_l0 = titles_l0 + ["Avizo fast",]
            
            # avizo_repeat_timations = [, , , ]
            # ax.plot([64**3, 151**3, 214**3, 253**3], avizo_repeat_timations, linestyle='--', marker='.', color='lime')
            # titles_l0 = titles_l0 + ["Avizo repeatable",]
        ...
        if D == 2:
            ...
            if LOGARIFMIC:
                x_unit = 'Image size, voxels (logarithmic scale)'
            else:
                x_unit = 'Image size, voxels'
            ax.set_xlabel(x_unit)
        plt.xticks(ticks[0], ticks[1])
        if LOGARIFMIC:
            time_unit = ', s (logarithmic scale)'
        else:
            time_unit = ', s'
        ax.set_ylabel('Processing time'+time_unit)
        if cluster == 'circles':
            cluster = 'cells'
        if LOGARIFMIC:
            title_plot = cluster
        ...
        if WITH_WL:
            plt.savefig('plots/'+cluster+'_WL.pdf',bbox_inches='tight')
        else:
            plt.savefig('plots/'+cluster+'.pdf',bbox_inches='tight')
