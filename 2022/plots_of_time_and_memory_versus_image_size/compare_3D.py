...
iterables = [['skimage', 'mahotas', 'smil', 'mamba', 'itk', 'ipsdk_fast', 'ipsdk_repeatable'],
             ['img_name', 'size', 'time']]
...
if __name__ == '__main__':
    ...
    for cur_img in image_generator(IMGS_FOLDER, filename, INIT_SIZE, [INIT_SIZE, *sizes_list]):
        print("Process", filename, 'size'+str(cur_img.size))

        cur_img.init_label(rad, indent)

        # IPSDK REPEATABLE PROCESSING
        if NEED_WL:
            ALGO = "ipsdk_repeatable"
